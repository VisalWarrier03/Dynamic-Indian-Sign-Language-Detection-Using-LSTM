...
BASE_DIR = r"C:\Users\visha\OneDrive\Desktop\U21CS010\VII Semester\Deep Learning HW\ISL Test\Final_Run_ver3"
...
def apply_temporal_augmentation(video_data, speed_factors=[ 0.9,1.1, 1.25, 1.5]):
    """
    Augment video data by varying the speed.
    Args:
        video_data: numpy array of shape (frames, features)
        speed_factors: list of speed multipliers
    Returns:
    ...
    for speed in speed_factors:
        if speed == 1.0:
            augmented_sequences.append(video_data)
            continue

        # Calculate new sequence length
        new_length = int(len(video_data) * (1 / speed))

        # Use linear interpolation to resize sequence
        indices = np.linspace(0, len(video_data) - 1, new_length)
        augmented_sequence = np.array([
            np.interp(indices, np.arange(len(video_data)), video_data[:, i])
...
def preprocess_data(csv_path, quick_train=False):
    """
    Preprocess data without augmentation.
    Args:
        csv_path: Path to the CSV file containing dataset metadata
        quick_train: Boolean flag for quick training with fewer classes
    Returns:
        features, labels, valid classes
    """
...
def apply_augmentation_to_dataset(features, labels, speed_factors=[0.75, 0.9, 1.1, 1.25, 1.5]):
    """
    Apply temporal augmentation to the dataset.
    Args:
        features: Original feature data
        labels: Original labels
        speed_factors: List of speed factors for augmentation
    Returns:
    ...
    for feature, label in zip(features, labels):
        augmented_sequences = apply_temporal_augmentation(feature, speed_factors)
        for aug_sequence in augmented_sequences:
            if aug_sequence.shape[0] < MAX_SEQUENCE_LENGTH:
                padding = np.zeros((MAX_SEQUENCE_LENGTH - aug_sequence.shape[0], FEATURE_DIM))
                aug_sequence = np.vstack([aug_sequence, padding])
            elif aug_sequence.shape[0] > MAX_SEQUENCE_LENGTH:
                aug_sequence = aug_sequence[:MAX_SEQUENCE_LENGTH]

            augmented_features.append(aug_sequence)
...
def create_model_variants(input_shape, num_classes):
    ...
    # Hierarchical Feature LSTM eith elu
    input_layer, mask_layer = get_input_layers()
    lstm1 = tf.keras.layers.LSTM(64, return_sequences=True)(mask_layer)
    pool1 = MaskedPooling(pooling_type='max')(lstm1)
    pool2 = MaskedPooling(pooling_type='avg')(lstm1)
    concat = tf.keras.layers.Concatenate()([pool1, pool2])
    dense1 = tf.keras.layers.Dense(64, activation='elu')(concat)
    dropout = tf.keras.layers.Dropout(0.2)(dense1)
    output = tf.keras.layers.Dense(num_classes, activation='softmax')(dropout)
    models["hierarchical_lstm_elu"] = tf.keras.Model(inputs=input_layer, outputs=output)
...
def main():
    ...
    # Preprocessing
    features, labels, classes = preprocess_data(CSV_PATH, quick_train=False)

    # Split data BEFORE augmentation
    x_train, x_val, y_train, y_val = train_test_split(features, labels, test_size=0.2, random_state=42)

    # Apply augmentation to training data
    x_train_aug, y_train_aug = apply_augmentation_to_dataset(x_train, y_train)

    # Combine original and augmented training data
    x_train = np.vstack([x_train, x_train_aug])
    y_train = np.vstack([y_train, y_train_aug])

    # Apply augmentation to validation data (optional, depends on use case)
    x_val_aug, y_val_aug = apply_augmentation_to_dataset(x_val, y_val)
    x_val = np.vstack([x_val, x_val_aug])
    y_val = np.vstack([y_val, y_val_aug])
    ...
    results = train_all_models(x_train, y_train, x_val, y_val, input_shape, num_classes, classes)
