...
MODEL_SAVE_PATH = r"C:\Users\visha\OneDrive\Desktop\U21CS010\VII Semester\Deep Learning HW\ISL Test\adjectives_lstm_model_17_2.keras"
...
def preprocess_data(csv_path, quick_train=False):
    """Processes dataset and prepares inputs and labels."""
...
def build_advanced_lstm_model(input_shape, num_classes):
    """Defines an advanced LSTM model."""
...
def train_and_evaluate(quick_train=False):
    ...
    # Apply temporal augmentation to validation data
    aug_features, aug_labels = temporal_augmentation(x_val, y_val)
    
    # Combine original and augmented training data
    x_val = np.vstack([x_train, aug_features])
    y_val = np.vstack([y_train, aug_labels])

    # Apply temporal augmentation to training data
    aug_features, aug_labels = temporal_augmentation(x_train, y_train)
    
    # Combine original and augmented training data
    x_train = np.vstack([x_train, aug_features])
    y_train = np.vstack([y_train, aug_labels])
